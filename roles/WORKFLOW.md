...
**LEAD is wired differently from the other six** — it isn't a row in `permissions.ts`'s `DEFAULT_MATRIX` at all. It's a DB-only role (`prisma/seed-add-lead-role.ts`) that sets `parentRoleId` to DEV and adds four `RolePermission` overrides on top (`task:assign: true`, `task:create: true`, `task:edit: true`, `workload:view-all: true`) — `task:create` so a lead can split one requirement into several task cards across the team without waiting on pm, `task:edit` so they can adjust priority/estimates on any task the team holds, not just their own. Every other action LEAD has (including `backlog:manage`, still `false`) resolves by walking up to DEV's own permissions at request time — so if DEV's rights ever change, LEAD's do too, automatically, with zero code changes. This is the same `parentRoleId` mechanism the schema's own comment names "Lead Dev" as the example for.
