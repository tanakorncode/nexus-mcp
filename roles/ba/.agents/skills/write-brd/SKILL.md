...
2. **Write it as a file in the repo**, not just as chat output — `docs/ba/brd-<short-name>.md` (create the folder if it doesn't exist). This is a project artifact meant to outlive the conversation, the same reason `nexus-pick-up-task` insists on `add_task_comment` over chat for anything that needs to persist.
